...
public class UserController {
    ...
    @GetMapping(value = {"/{id}", ""})
    public ResponseEntity<UserDto> getUser(@PathVariable(name = "id") Optional<Long> id) {
        return new ResponseEntity<>(userService.userById(id.orElse(null)), HttpStatus.OK);
    }


    @GetMapping(value = {"/{start}/{end}"})
    public ResponseEntity<List<UserDto>> usersRegisteredBetween(@PathVariable(name = "start") String start,
    ...
    public ResponseEntity<List<UserDto>> usersRegisteredBetween(@RequestParam(name = "start", required = false) Optional<String> start,
